...
class Level_14: Level {
    ...
    override func didMove(to view: SKView) {
        ...
        print("did move of level 14")
        
        //level 14 has more obstacles which fall from the ceiling, so we need to zoom the camera out in order for the player to be able to see them
        let zoomInAction = SKAction.scale(to: 2, duration: 0)
        cameraNode.run(zoomInAction)
        
        //play the correct sound
        playSound(nameOfFile: "Level_Music_14", type: "mp3")
    }
    ...
    override func updateCamera() {
        if(currentPlayer != nil && currentPlayer?.position != nil) {
            ...
            let y = clamp(value: currentPlayer.position.y, lower: UIScreen.main.bounds.width/4, upper: 1*UIScreen.main.bounds.width)
            let x = clamp(value: currentPlayer.position.x, lower: (UIScreen.main.bounds.height)/2 + 100 , upper: (4*(UIScreen.main.bounds.height/2)))
    ...
    func reformY(y: CGFloat) -> CGFloat {
        return clamp(value: y, lower: UIScreen.main.bounds.width/4, upper: 5*UIScreen.main.bounds.width)
    }
    
    func reformX(x: CGFloat) -> CGFloat {
        return clamp(value: x, lower: (UIScreen.main.bounds.height)/2 + 100 , upper: (7*(UIScreen.main.bounds.height/2) + 190))
    }
}
