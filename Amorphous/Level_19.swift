...
class Level_19: Level {
    ...
    override func didMove(to view: SKView) {
        ...
        print("did move of level 19")
        
        //level 19 has more obstacles which fall from the ceiling, so we need to zoom the camera out in order for the player to be able to see them
        let zoomInAction = SKAction.scale(to: 2, duration: 0)
        cameraNode.run(zoomInAction)
        
        //play the correct sound
        playSound(nameOfFile: "Level_Music_19", type: "mp3")
    }
    ...
    override func updateCamera() {
        if(currentPlayer != nil && currentPlayer?.position != nil) {
            ...
            let x = clamp(value: currentPlayer.position.x, lower: (UIScreen.main.bounds.height)/2 + 100 , upper: (19*(UIScreen.main.bounds.height/2) + 190))
    ...
    func reformX(x: CGFloat) -> CGFloat {
        return clamp(value: x, lower: (UIScreen.main.bounds.height)/2 + 100 , upper: (19*(UIScreen.main.bounds.height/2) + 190))
    }
}
