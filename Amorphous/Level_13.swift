...
class Level_13: Level {
    ...
    override func didMove(to view: SKView) {
        ...
        print("did move of level 13")
        
        //level 13 has more obstacles which fall from the ceiling, so we need to zoom the camera out in order for the player to be able to see them
        let zoomInAction = SKAction.scale(to: 2, duration: 0)
        cameraNode.run(zoomInAction)
        
        //play the correct sound
        playSound(nameOfFile: "Level_Music_13", type: "mp3")
    }
