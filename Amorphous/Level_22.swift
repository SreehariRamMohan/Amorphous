...
//  Amorphous
//
//  Created by Sreehari Ram Mohan on 7/17/17.
...
class Level_22: Level {
    ...
    override func didMove(to view: SKView) {
        ...
        print("did move of level 5")
        
        //level 5 has more obstacles, so we need to zoom the camera out in order for the player to be able to see them
        let zoomInAction = SKAction.scale(to: 2, duration: 1)
        cameraNode.run(zoomInAction)
        
        //play the correct sound
        playSound(nameOfFile: "Level_Music_22", type: "mp3")
    }
    ...
    override func updateCamera() {
        if(currentPlayer != nil && currentPlayer?.position != nil) {
            ...
            let x = clamp(value: currentPlayer.position.x, lower: (UIScreen.main.bounds.height)/2 + 100 , upper: (11*(UIScreen.main.bounds.height/2) + 320))
    ...
    func reformY(y: CGFloat) -> CGFloat {
        return clamp(value: y, lower: UIScreen.main.bounds.width/4, upper: UIScreen.main.bounds.width/2 - 180)
    }
    
    func reformX(x: CGFloat) -> CGFloat {
        return clamp(value: x, lower: (UIScreen.main.bounds.height)/2 + 100 , upper: (13*(UIScreen.main.bounds.height/2) + 190))
    }
