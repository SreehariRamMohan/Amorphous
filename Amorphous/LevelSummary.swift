...
class LevelSummary: SKSpriteNode {
    ...
    func populateWithInformation() {
        ...
        //currently we don't have more than 25 levels so we need to stop the player from hitting onward at level 26 since this will crash the game/make it unusable since the level indexing will be completely messed up. 
        if(LevelSelect.current_level == Level.NUMBER_OF_TOTAL_LEVELS) {
