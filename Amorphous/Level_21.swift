...
class Level_21: Level {
    ...
    override func didMove(to view: SKView) {
        ...
        print("did move of level 21")
        
        //level 21 has more obstacles which fall from the ceiling, so we need to zoom the camera out in order for the player to be able to see them
        let zoomInAction = SKAction.scale(to: 2, duration: 0)
        cameraNode.run(zoomInAction)
        
        //play the correct sound
        playSound(nameOfFile: "Level_Music_21", type: "mp3")
    }
    ...
    override func updateCamera() {
        if(currentPlayer != nil && currentPlayer?.position != nil) {
            ...
            let y = clamp(value: currentPlayer.position.y, lower: UIScreen.main.bounds.width/6, upper: 2*UIScreen.main.bounds.width)
    ...
    func reformY(y: CGFloat) -> CGFloat {
        return clamp(value: y, lower: UIScreen.main.bounds.width/6, upper: 2*UIScreen.main.bounds.width)
    }
