...
class Level_3: Level {
    ...
    override func didMove(to view: SKView) {
        ...
        playSound(nameOfFile: "Level_Music_3", type: "mp3")
    }
    ...
    override func updateCamera() {
        if(currentPlayer != nil && currentPlayer?.position != nil) {
            ...
            let x = clamp(value: currentPlayer.position.x, lower: 20 , upper: 3*(UIScreen.main.bounds.width/2 - 100))
    ...
    func reformY(y: CGFloat) -> CGFloat {
        return clamp(value: y, lower: UIScreen.main.bounds.width/20, upper: UIScreen.main.bounds.width/2-10)
    }
    
    func reformX(x: CGFloat) -> CGFloat {
        return clamp(value: x, lower: CGFloat(20), upper: 3*(UIScreen.main.bounds.width/2 - 100))
    }
}
