...
class Level_25: Level {
    ...
    override func didMove(to view: SKView) {
        ...
        print("did move of level 25")
        
        //level 25 has more obstacles, so we need to zoom the camera out in order for the player to be able to see them
        let zoomInAction = SKAction.scale(to: 2, duration: 0)
        cameraNode.run(zoomInAction)
        
        //play the correct sound
        playSound(nameOfFile: "Level_Music_25", type: "mp3")
    }
    ...
    override func updateCamera() {
        if(currentPlayer != nil && currentPlayer?.position != nil) {
            ...
            var y = clamp(value: currentPlayer.position.y, lower: UIScreen.main.bounds.width/4, upper: 3*UIScreen.main.bounds.width - 180)
    ...
    func reformY(y: CGFloat) -> CGFloat {
        return clamp(value: y, lower: UIScreen.main.bounds.width/4, upper: 3*UIScreen.main.bounds.width - 180)
    }
