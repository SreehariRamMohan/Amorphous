...
//  Amorphous
//
//  Created by Sreehari Ram Mohan on 7/23/17.
...
class Level_9: Level {
    ...
    override func updateCamera() {
        if(currentPlayer != nil && currentPlayer?.position != nil) {
            ...
            let x = clamp(value: currentPlayer.position.x, lower: (UIScreen.main.bounds.height)/2 + 100 , upper: (25*(UIScreen.main.bounds.height/2) + 190))
    ...
    func reformX(x: CGFloat) -> CGFloat {
        return clamp(value: x, lower: (UIScreen.main.bounds.height)/2 + 100 , upper: (25*(UIScreen.main.bounds.height/2) + 190))
    }
}
